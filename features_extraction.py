            ...
            if number_in_range(int(enter_time), start_time, end_time) and location_id == location_name:
                ...
                if data_unlock.__len__() > 0:
                    for item_unlock in data_unlock:
                        ...
                        if number_in_range(int(start), int(enter_time), int(exit_time)) and number_in_range(
                                int(end), int(enter_time), int(exit_time)):
                            result += int(duration)
    return result if result > 0 else "-"
...
def get_app_category_usage(self, dataset, start_time, end_time, user_email, day_num, ema_no):
    ...
    data = list(dataset)
    app_pkg_dict = {"Entertainment_Music": {}, "Utilities": {}, "Shopping": {}, "Games_Comics": {}, "Others": {},
                    "Health_Wellness": {},
                    "Social_Communication": {}, "Education": {}, "Travel": {}, "Art_Photo": {}, "News_Magazine": {},
                    "Food_Drink": {}}
    # TODO : Application package
    if data.__len__() > 0:
        for item in data:
        ...
        pkg_most = {"Entertainment_Music": "음악_및_영상", "Utilities": "클라우드_및_문서도구", "Shopping": "결제_및_쇼핑",
                    "Games_Comics": "게임_및_웹툰",
                    "Others": "비즈니스_도구(취업_및_화상미팅)", "Health_Wellness": "건강_관리_도구",
                    "Social_Communication": "SNS_및_메일", "Education": "교육_관련_앱", "Travel": "교통_도구(지도)",
                    "Art_Photo": "사진 ",
        ...
        try:
            AppUsed.objects.create(uid=self.uid, day_num=day_num, ema_order=ema_no,
                                   Entertainment_Music=pkg_most['Entertainment_Music'],
                                   Utilities=pkg_most['Utilities'], Shopping=pkg_most['Shopping'],
                                   Games_Comics=pkg_most['Games_Comics'], Others=pkg_most['Others'],
                                   Health_Wellness=pkg_most['Health_Wellness'],
                                   Social_Communication=pkg_most['Social_Communication'],
                                   Education=pkg_most['Education'],
                                   Travel=pkg_most['Travel'], Art_Photo=pkg_most['Art_Photo'],
                                   News_Magazine=pkg_most['News_Magazine'],
    ...
    try:
        if data.__len__() > 0:
            for index in range(0, len(data) - 1):
                ...
                lat2 = values_next[1]
                lng2 = values_next[2]
                if number_in_range(int(time1), start_time, end_time) and number_in_range(int(time2), start_time,
                                                                                         end_time):
                    ...
                    if not location_coordinates["lat"] == -1:  # enough to check only lat
                        distance_from_home = get_distance(location_coordinates["lat"], location_coordinates["lng"],
                        ...
                        result[
                            'max_dist_from_home'] = '-'  # max dist from home is unknown if lat or lng of location is -1
...
def get_survey_data(self, ema_order, end_time):
    ...
    if ema_array.__len__() > 0:
        for ema in ema_array:
            ...
            if order == ema_order and number_in_range(answered_time, end_time,
                                                      end_time + self.EMA_RESPONSE_EXPIRE_TIME * 1000):
...
def extract_regular(self, start_ts, end_ts, ema_order, user_email, day_num):
    ...
    try:
        ...
        activities_total_dur = self.get_activities_dur_result(self.dataset[self.ACTIVITY_TRANSITION], start_ts,
                                                              end_ts)
        dif_activities = self.get_num_of_dif_activities_result(self.dataset[self.ACTIVITY_RECOGNITION], start_ts,
                                                               end_ts)
        ...
        app_usage = self.get_app_category_usage(dataset=self.dataset[self.APPLICATION_USAGE], start_time=start_ts,
                                                end_time=end_ts, user_email=user_email, day_num=day_num,
                                                ema_no=ema_order)
        day_hour_start = 18
        day_hour_end = 10
        ...
        sleep_duration = self.get_sleep_duration(self.dataset[self.SCREEN_ON_OFF], date_start.timestamp() * 1000,
                                                 date_end.timestamp() * 1000)
    ...
    try:
        ...
        if ema_responses.__len__() > 0:  # EMA_responses가 있을 때만 feature extract 진행
            ...
            for index, ema_res in enumerate(ema_responses):
                ...
                activities_total_dur = self.get_activities_dur_result(self.dataset[self.ACTIVITY_TRANSITION],
                                                                      start_time, end_time)
                dif_activities = self.get_num_of_dif_activities_result(self.dataset[self.ACTIVITY_RECOGNITION],
                                                                       start_time, end_time)
                audio_data = self.get_audio_data_result(self.dataset[self.AUDIO_LOUDNESS], start_time, end_time)
                time_at = self.get_time_at_location(self.dataset[self.GEOFENCE], start_time, end_time,
                                                    self.LOCATION_HOME)
                coordinates = self.get_location_coordinates(self.dataset[self.LOCATIONS_MANUAL], self.LOCATION_HOME)
                gps_data = self.get_gps_location_data(self.dataset[self.LOCATION_GPS], coordinates, start_time,
                                                      end_time)
                unlock_at = self.get_unlock_duration_at_location(
                    self.dataset[self.GEOFENCE],
                    self.dataset[self.UNLOCK_DURATION],
                ...
                #     self.dataset[self.AUDIO_LOUDNESS],
                #     start_time, end_time)
                app_usage = self.get_app_category_usage_at_first(self.dataset[self.APPLICATION_USAGE], start_time,
                                                                 end_time)
                ## Sleep duration 계산
                day_hour_start = 18
                day_hour_end = 10
                ...
                date_end = date_end.replace(hour=day_hour_end, minute=0, second=0)
                sleep_duration = self.get_sleep_duration(self.dataset[self.SCREEN_ON_OFF],
